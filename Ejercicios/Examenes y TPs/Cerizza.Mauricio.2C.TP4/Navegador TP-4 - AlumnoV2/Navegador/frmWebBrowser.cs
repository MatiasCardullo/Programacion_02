...
namespace Navegador
{
    public partial class frmWebBrowser : Form
    {
        ...
        {
            this.ActualizarUrl(); //Agrega http:// si es necesario.
            Thread t1 = new Thread(this.ActualizarHistorial);  //Guarda la dirección en el historial.          
            Thread t2 = new Thread(this.Descargar); //Comienza la descarga de la página. 
            t1.Start();
            t2.Start();
        }
        ...
        {
            if(!(Regex.IsMatch(txtUrl.Text, @"^(http:\/\/|https:\/\/)(.*)$")))
            {
        ...
        private void Descargar()
        {
            try
            {
                ...
                descargador.descargaEnProgreso += this.ProgresoDescarga;
            }
            catch (Exception ex)
            {
                MessageBox.Show("ERROR: " + ex.Message);
